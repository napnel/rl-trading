...
from utils import collect_random, save


def get_config():
    parser = argparse.ArgumentParser(description="RL")
    parser.add_argument(
        "--run_name", type=str, default="CQL-DQN", help="Run name, default: CQL-DQN"
    )
    parser.add_argument(
        "--env",
        type=str,
    ...
    parser.add_argument(
        "--episodes", type=int, default=300, help="Number of episodes, default: 200"
    )
    parser.add_argument(
        "--buffer_size",
        type=int,
    ...
    parser.add_argument(
        "--min_eps", type=float, default=0.01, help="Minimal Epsilon, default: 4"
    )
    parser.add_argument(
        "--eps_frames",
        type=int,
        default=1e3,
        help="Number of steps for annealing the epsilon value to the min epsilon, default: 1e-5",
    )
    parser.add_argument(
        "--log_video",
        type=int,
...
def train(config):
    ...
    with wandb.init(project="CQL", name=config.run_name, config=config):
        ...
        buffer = ReplayBuffer(
            buffer_size=config.buffer_size, batch_size=32, device=device
        )
        ...
        if config.log_video:
            env = gym.wrappers.Monitor(
                env, "./video", video_callable=lambda x: x % 10 == 0, force=True
            )
        ...
        for i in range(1, config.episodes + 1):
            ...
            print(
                "Episode: {} | Reward: {} | Q Loss: {} | Steps: {}".format(
                    i,
                    rewards,
                    loss,
                    steps,
            ...
            if i % config.save_every == 0:
                save(
                    config, save_name="CQL-DQN", model=agent.network, wandb=wandb, ep=0
                )
